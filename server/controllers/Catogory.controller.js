...
exports.getAllCatogories=async(req,res)=>{
    try{
        const catogories=await Catogory.find({},{name:true,description:true});
        if(!catogories){
            return res.status(400)
            .json({
                success:false,
...
exports.categoryPageDetails=async(req,res)=>{
    try {
        ...
        const differentCategory=await Catogory.find({_id:{$ne:category_id}}).populate('course').exec();

        const topcourse=await Course.find({}).sort({studentEnrolled:-1}).limit(5);
        res.status(200)
        .json({
            success:true,
            data:{
